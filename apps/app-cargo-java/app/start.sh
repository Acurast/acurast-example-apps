...
# Entry point for the Shell (cargo) runtime. Sets up the rootfs, installs a
# JDK, then runs the program which POSTs a JSON payload to WEBHOOK_URL.
...
export WEBHOOK_URL="${WEBHOOK_URL%/}/java"
...
report() {
    ...
    curl -sS -X POST "$WEBHOOK_URL" -H "Content-Type: application/json" \
        -d "{\"language\":\"java\",\"status\":\"$1\"${2:+,$2}}" >/dev/null 2>&1 || true
}
fail() {
...
apt-get install -y default-jdk 2>"$HOME/setup.err" || fail "setup" $? "$HOME/setup.err"

# Single-file source-code launch (JDK 11+): compiles and runs in one step.
java "$SCRIPT_DIR/Main.java" 2>"$HOME/run.err" || fail "run" $? "$HOME/run.err"
