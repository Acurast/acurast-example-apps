#!/usr/bin/env python3
"""Acurast reverse-tunnel client for the Minecraft deployment.
...
local Minecraft server; the SECONDARY (self-signed) forwards to a local dropbear
SSH instance. The easy way to play is to SSH in over the secondary connection
and local-forward the game port (ssh -L) — Minecraft's wire protocol is raw TCP,
so the SSH session carries it without any TLS wrapper on the client.
"""
...
if not DOMAIN_SUFFIX:
    ...
    sys.exit(1)
# Primary (ACME) tunnel serves the Minecraft server; secondary (self-signed) maps to SSH.
MC_PORT = int(os.environ.get("MC_PORT", "25565"))
SSH_PORT = int(os.environ.get("SSH_PORT", "2222"))
LOCAL_ADDR = f"127.0.0.1:{MC_PORT}"
...
def main():
    ...
    spec = {
        ...
        # Primary (ACME) connection forwards here — the Minecraft server.
    ...
    report_log(f"Requesting reverse tunnel (minecraft -> {LOCAL_ADDR}, ssh -> {SECONDARY_LOCAL_ADDR})")
    ...
    report_log(f"Tunnel started: minecraft url={url} clientId={client_id}")
