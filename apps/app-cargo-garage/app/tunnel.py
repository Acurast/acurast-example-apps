#!/usr/bin/env python3
"""Acurast reverse-tunnel client for the Garage S3 deployment.
...
local Garage S3 API so it's reachable by any S3 client with a valid cert; the
...
# Primary (ACME) tunnel serves the Garage S3 API; secondary (self-signed) maps to SSH.
...
def main():
    ...
    spec = {
        ...
        # Primary (ACME) connection forwards here — the Garage S3 API.
    ...
    report_log(f"Requesting reverse tunnel (s3 -> {LOCAL_ADDR}, ssh -> {SECONDARY_LOCAL_ADDR})")
    ...
    report_log(f"Tunnel started: s3 url={public_url} clientId={client_id}")
    ...
    print(f"S3 endpoint is live at:\n  {public_url}")
    if connect_cmd:
