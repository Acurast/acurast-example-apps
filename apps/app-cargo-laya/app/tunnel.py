#!/usr/bin/env python3
"""Acurast reverse-tunnel client for the Laya deployment.
...
local Laya HTTP API; the SECONDARY (self-signed) forwards to a local dropbear
SSH instance for shell access (debugging).
"""
...
# Primary (ACME) tunnel serves the Laya API; secondary (self-signed) maps to SSH.
...
def main():
    ...
    spec = {
        ...
        # Primary (ACME) connection forwards here — the Laya API.
    ...
    if not ssh_client_id:
        report_error(
            ...
            "secondaryLocalAddr support; SSH access will not be reachable."
        )
        connect_cmd = None
        forward_cmd = None
    else:
        ...
        # debug a failed deployment). `forward_cmd` forwards the Laya port locally,
        # handy while the primary (ACME) cert is still being issued.
        proxy = (
        ...
        forward_cmd = f"ssh -N -L {WEB_PORT}:127.0.0.1:{WEB_PORT} {proxy} root@{ssh_client_id}"

    report_started(web_url, ssh_url, SSH_PORT, connect_cmd, forward_cmd)
    print(f"Laya API (POST /v1/systemone, GET /health):\n  {web_url}")
    if connect_cmd:
        print(f"SSH shell (debug):\n  {connect_cmd}")
        print(f"Laya API via SSH local-forward:\n  {forward_cmd}")
