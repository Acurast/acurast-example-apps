...
- `start.sh` sets up `PATH`/`HOME`/DNS, installs `ruby` via `apt-get`, then
  runs the program with `ruby`.
- `main.rb` builds a JSON body (`language`, `runtime`, `message`) and POSTs it
  to `WEBHOOK_URL` (with a `/ruby` subpath for attribution) using `net/http`.
...
- Uses Ruby's standard `net/http` library — no gems to install.
