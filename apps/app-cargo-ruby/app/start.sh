...
# Entry point for the Shell (cargo) runtime. Sets up the rootfs, installs Ruby,
# then runs the program which POSTs a JSON payload to WEBHOOK_URL.
...
export WEBHOOK_URL="${WEBHOOK_URL%/}/ruby"
...
report() {
    ...
    curl -sS -X POST "$WEBHOOK_URL" -H "Content-Type: application/json" \
        -d "{\"language\":\"ruby\",\"status\":\"$1\"${2:+,$2}}" >/dev/null 2>&1 || true
}
fail() {
...
apt-get install -y ruby 2>"$HOME/setup.err" || fail "setup" $? "$HOME/setup.err"

ruby "$SCRIPT_DIR/main.rb" 2>"$HOME/run.err" || fail "run" $? "$HOME/run.err"
