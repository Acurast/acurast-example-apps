...
# PHP CLI, then runs the program which POSTs a JSON payload to WEBHOOK_URL.
...
export WEBHOOK_URL="${WEBHOOK_URL%/}/php"
...
report() {
    ...
    curl -sS -X POST "$WEBHOOK_URL" -H "Content-Type: application/json" \
        -d "{\"language\":\"php\",\"status\":\"$1\"${2:+,$2}}" >/dev/null 2>&1 || true
}
fail() {
...
apt-get install -y php-cli php-curl 2>"$HOME/setup.err" || fail "setup" $? "$HOME/setup.err"

php "$SCRIPT_DIR/main.php" 2>"$HOME/run.err" || fail "run" $? "$HOME/run.err"
