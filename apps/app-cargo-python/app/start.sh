...
# Python requests library, then runs the program which POSTs a JSON payload to
# WEBHOOK_URL.
...
export WEBHOOK_URL="${WEBHOOK_URL%/}/python"
...
apt-get install -y python3-requests 2>"$HOME/setup.err" || fail "setup" $? "$HOME/setup.err"

python3 "$SCRIPT_DIR/main.py" 2>"$HOME/run.err" || fail "run" $? "$HOME/run.err"
